...
function renderBlock(block, i) {
  switch (block.type) {
    ...
    case "p":
      return <p key={i} className="text-sm text-[#1A1A1A]/65 leading-relaxed mb-4">{block.text}</p>;
    case "ul":
      return (
        <ul key={i} className="space-y-2 mb-4 pl-4">
          {block.items.map((item, j) => (
            <li key={j} className="flex items-start gap-2 text-sm text-[#1A1A1A]/65 leading-relaxed">
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          >
            <motion.div variants={fadeInUp}>
              <Link
                href="/blog"
                className="inline-flex items-center gap-2 text-xs font-semibold text-[#C9B8A8] hover:text-[#1A1A1A] transition-colors duration-200 mb-8 cursor-pointer"
              >
            ...
            <motion.p variants={fadeInUp} className="text-base text-[#1A1A1A]/60 leading-relaxed bg-[#6B1E2E]/5 px-5 py-4 italic">
              {article.excerpt}
            </motion.p>
