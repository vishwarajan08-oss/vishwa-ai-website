          ...
          >
            {allArticles.map((article) => (
              <motion.article
                key={article.slug}
                variants={slideUp}
                className="group bg-white border border-[#E8E0DA] hover:border-[#6B1E2E] hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer overflow-hidden"
                style={{ boxShadow: "0 0 0 0 rgba(107,30,46,0)" }}
                whileHover={{ boxShadow: "0 4px 20px rgba(107,30,46,0.08)" }}
              >
